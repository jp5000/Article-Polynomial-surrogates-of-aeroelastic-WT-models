...
\eq{\hat{y}_{\expect}(\vx)\approx y_{\expect}(\vx)=\expect(y(\vx))}{}

\noindent and a PCE for the local standard deviation:

\eq{\hat{y}_{\std}(\vx) \approx y_{\std}(\vx)=\sqrt{\variance(y(\vx))}=\std(y(\vx))}{}
%% What do you mean by local statistical moments? They are usually denoted as raw moments (e.g. the mean), and central moments (when the mean is subtracted, as in e.g. calculating the variance). Or is the meaning something else, e.g. location in the domain of input variables? Maybe it should be explained better. 
...
\eq{c_l = \langle  y,  \phi_{l} \rangle  =\int y(x)\, \phi_{l}(x)\, \pdf(x)\,dx \approx \sum_{i=0}^{N_n} \omega_i \, y(x_i) \, \phi_{l}(x_i)}{eq_projection}
